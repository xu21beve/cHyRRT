...
base::PlannerStatus ompl::geometric::HyRRT::solve(const base::PlannerTerminationCondition &ptc)
{
    ...
    while (!ptc())
    {
        ...
        switch (priority)
        {
            case false:  // Flow
                ...
                while (tFlow < randomFlowTimeMax && flowSet_(new_motion->state))
                {
                    ...
                    new_state =
                        this->flowPropagation_(flowInputs, previous_state, flowStepLength_, new_state);
        ...
        jump:
            ...
            base::State *new_state = si_->allocState();
            new_state = this->jumpPropagation_(new_motion->state, jumpInputs, new_state); // changed from previous_state to new_motion->state
